  ...
  return (
    ...
    <aside className="fixed bottom-0 left-0 right-0 z-50 md:left-0 md:top-0 md:h-screen md:w-80 flex flex-col items-center py-4 md:py-8 pointer-events-none">
      ...
      <div className="pointer-events-auto bg-[#0a0a0a]/90 backdrop-blur-2xl border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.8)] 
                      w-[95%] md:w-full md:h-[95%] md:my-auto md:ml-6 md:rounded-[40px] rounded-2xl mb-4 md:mb-0
                      flex flex-row md:flex-col justify-between p-2 md:p-8 overflow-hidden relative">
        
        {/* Decorative Blur Glow inside sidebar */}
        <div className="absolute -top-10 -left-10 w-32 h-32 bg-neon-glow rounded-full blur-[50px] opacity-10 pointer-events-none"></div>

        {/* Brand */}
        <div className="hidden md:flex items-center gap-4 px-2 mb-8">
        ...
        {/* Navigation Items */}
        <nav className="flex md:flex-col w-full gap-1 md:gap-4 justify-around md:justify-start">
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`
                  relative group flex items-center gap-4 px-4 py-3 md:py-4 rounded-xl md:rounded-2xl w-full transition-all duration-300 overflow-hidden
                  ${isActive 
              ...
              >
                ...
                {/* Mobile Active Dot */}
                {isActive && (
                  <div className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-neon rounded-full shadow-[0_0_5px_#39ff14] md:hidden block"></div>
                )}
                ...
                <span className={`
                  hidden md:block text-sm font-medium relative z-10 transition-all duration-300
